...
const getSchedule = () => {
    ...
    if ($(".selected").length > 0) {
        ...
        do {
            ...
            let startInterval = $(intervals).eq(0).hasClass("selected")
                ? $(intervals).eq(0)
...
const setUpTextAndColors = (start, end, button) => {
    for (let i = start; i <= end; i++) {
        ...
        $(`.interval-button[data-id="${i}"]`)
            .text("")
            .closest(".interval")
            .addClass("selected");
    }
    let firstButtonCheck = $(button)
        .closest(".interval")
        .prevUntil(":not(.selected)")
        .find(".interval-button")
        .text("");
    let lastButtonCheck = $(button)
        .closest(".interval")
        .nextUntil(":not(.selected)")
        .find(".interval-button")
        .text("");
    ...
    let changeTextButton =
        firstButtonCheck.length !== 0 ? firstButtonCheck.first() : button;
    let lastButtonChecked =
        lastButtonCheck.length !== 0 ? lastButtonCheck.last() : button;

    let text = `Доступен с ${$(changeTextButton)
        .closest(".interval")
        .find(".interval-text")
        .text()} до ${$(lastButtonChecked).attr("hour")}`;
...
for (let i = 0; i <= 24; i++) {
    ...
    const timeSpan = $("<span>")
        .text(`${i < 10 ? "0" : ""}${i}:00`)
        .addClass("interval-text");

    const buttonContainer = $("<div>").addClass("button-container");
    const button = $("<button>")
        .addClass("interval-button")
        .attr("data-id", i)
        .attr("hour", `${i + 1 < 10 ? "0" : ""}${i + 1}:00`);
